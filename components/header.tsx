...
import { Menu, Search, ShoppingBag, X, ChevronRight, ChevronDown, ChevronLeft } from "lucide-react"
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      ...
      try {
        ...
        if ((err as Error).name !== "AbortError") console.error("Error fetching categories:", err)
      } finally {
  ...
  const handleCategoriesLeave = () => {
    categoriesTimeoutRef.current = setTimeout(() => setIsCategoriesOpen(false), 150)
  }
  ...
  return (
    <header className="fixed top-0 left-0 right-0 w-full z-50">
      ...
      {(siteConfig.features as Record<string, boolean>).multiLanguage && (
        <div
      ...
      <div className={`max-w-7xl mx-auto px-4 flex items-center relative z-10 transition-all duration-200 ${isScrolled || alwaysVisible ? "h-[64px] lg:h-[72px]" : "h-[76px] lg:h-[90px]"
        ...
        {/* Mobile hamburger */}
        <button
          className={`transition-all duration-300 lg:hidden relative z-50 p-2 -ms-2 ${mobileIconClass}`}
        ...
        <nav className={`hidden lg:flex items-center gap-2 transition-all duration-200 ${alwaysVisible || isScrolled
          ...
          <DesktopNavLink href="/" label={t("home")} />
          <DesktopNavLink href="/a-propos" label={t("about")} />
          ...
          >
            ...
            >
              ...
              <ChevronDown className={`w-4 h-4 transition-transform duration-300 relative z-10 ${isCategoriesOpen ? "rotate-180" : ""}`} />
            ...
            {isCategoriesOpen && (
              <div className="absolute top-full start-0 mt-2 w-56 max-h-[60vh] overflow-y-auto bg-[#FCFAF8] rounded-bl-2xl rounded-br-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-[#E5DDD3]/60 py-3 animate-in fade-in zoom-in-95 duration-300 z-50 overflow-hidden">
                ...
                {loadingCategories ? (
                  ...
                  primaryCategories.map((category) => (
                    ...
                    >
                      ...
                      <ArrowIcon className="w-3.5 h-3.5 text-[#B0A8A0] opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all duration-300" />
                    </Link>
                  ))
                )}
              </div>
          ...
          <DesktopNavLink href="/products" label={t("products")} />
        </nav>
        ...
        >
          <div className="relative">
            <div className={`absolute inset-0 bg-white/20 blur-xl rounded-full transition-opacity duration-200 ${isHomePage && !isScrolled ? "opacity-100" : "opacity-0"}`} />
            <Image
        ...
        <div className="flex items-center gap-2 lg:hidden relative z-50">
          ...
          <Link href="/cart" className={`relative transition-all duration-300 p-2 group ${mobileIconClass}`}>
            ...
            {cartCount > 0 && (
              <span className="absolute top-0 end-0 bg-[#8b7e6e] text-white text-[10px] rounded-[40%_60%_70%_30%/40%_50%_60%_50%] w-[16px] h-[16px] flex items-center justify-center font-bold shadow-sm group-hover:bg-[#1a1a1a] transition-colors">
                {cartCount}
              </span>
            )}
          </Link>
        ...
        {/* Actions - DESKTOP */}
        <div className={`hidden lg:flex items-center gap-4 justify-self-end relative z-50 transition-all duration-200 ${alwaysVisible || isScrolled
          ? "opacity-100 translate-y-0 pointer-events-auto"
          : "opacity-0 -translate-y-2 pointer-events-none"
          }`}>
          ...
          <Link href="/cart" className="relative p-1 transition-all duration-300 hover:scale-110 group text-[#4A4A4A] hover:text-[#1a1a1a]">
            <ShoppingBag className="w-5 h-5 stroke-[1.5]" />
            {cartCount > 0 && (
              <span className="absolute -top-2 -end-2.5 bg-[#8b7e6e] text-white text-[10px] rounded-[40%_60%_70%_30%/40%_50%_60%_50%] w-[18px] h-[18px] flex items-center justify-center font-bold shadow-sm group-hover:bg-[#1a1a1a] transition-colors">
                {cartCount}
              </span>
            )}
          </Link>
      ...
      {isSearchOpen && (
        <form onSubmit={handleSearch} className="absolute w-full px-4 py-4 animate-in slide-in-from-top-2 fade-in duration-300 z-0">
          ...
          <div className="max-w-2xl mx-auto relative group">
            <Search className="absolute start-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#8b7e6e] transition-colors group-focus-within:text-[#1a1a1a]" />
            <input
              type="text"
              value={searchTerm}
              ...
              autoFocus
              className="w-full ps-12 pe-4 py-3.5 bg-white/50 border border-[#E5DDD3] text-[#2D2D2D] text-sm placeholder:text-[#999] focus:outline-none focus:border-[#8b7e6e] focus:bg-white transition-all shadow-inner rounded-sm"
      ...
      {isMenuOpen && (
        <>
          <div
            ...
            onClick={closeMenu}
          />
          <nav className={`fixed top-0 bottom-0 w-[85vw] max-w-[320px] bg-[#FCFAF8] shadow-[10px_0_30px_rgba(0,0,0,0.1)] z-50 overflow-y-auto flex flex-col lg:hidden border-e border-[#E5DDD3]/50 animate-in duration-500 ${isRTL ? "right-0 slide-in-from-right" : "left-0 slide-in-from-left"}`}>
            ...
            <form onSubmit={handleSearch} className="px-6 pb-6">
              <div className="relative group">
                <input
                  ...
                  className="w-full pe-10 ps-4 py-3 bg-white/60 border border-[#E5DDD3] text-sm text-[#2D2D2D] placeholder:text-[#999] focus:outline-none focus:border-[#8b7e6e] transition-colors"
                  style={{ borderRadius: "255px 15px 225px 15px/15px 225px 15px 255px" }}
                />
                <button type="submit" className="absolute end-3 top-1/2 -translate-y-1/2">
            ...
            <div className="flex px-6 mb-2">
              <button
                ...
                className={`flex-1 py-2 text-[11px] font-bold tracking-[0.2em] uppercase text-center transition-all duration-300 relative ${activeTab === "menu" ? "text-[#1a1a1a]" : "text-[#999] hover:text-[#4A4A4A]"}`}
              >
                {t("menu")}
                {activeTab === "menu" && (
              ...
              <button
                ...
                className={`flex-1 py-2 text-[11px] font-bold tracking-[0.2em] uppercase text-center transition-all duration-300 relative ${activeTab === "categories" ? "text-[#1a1a1a]" : "text-[#999] hover:text-[#4A4A4A]"}`}
              >
                {t("categories")}
            ...
            <div className="flex-1 overflow-y-auto px-2">
              {activeTab === "menu" ? (
                <div className="py-2">
                  <SidebarLink href="/" label={t("home").toUpperCase()} onClick={closeMenu} />
                  <SidebarLink href="/a-propos" label={t("about").toUpperCase()} onClick={closeMenu} />
                  <SidebarLink href="/products" label={t("products").toUpperCase()} onClick={closeMenu} hasArrow isRTL={isRTL} />
                  <SidebarLink href="/products" label={t("newCollection").toUpperCase()} onClick={closeMenu} />
                  <SidebarLink href="#about-contact" label={t("contact").toUpperCase()} onClick={closeMenu} />
                  {siteConfig.social.instagram.url && (
                    <a
                      ...
                      onClick={closeMenu}
                    >
                      <span>{t("instagram").toUpperCase()}</span>
                      <ArrowIcon className="w-3.5 h-3.5 text-[#B0A8A0] opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                    </a>
                  )}
                  ...
                  {loadingCategories ? (
                    ...
                    <>
                      <Link
                        ...
                        onClick={closeMenu}
                      >
                        <span>{t("allProducts").toUpperCase()}</span>
                        <ArrowIcon className="w-3.5 h-3.5 text-[#B0A8A0] opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                      </Link>
                      {primaryCategories.map((category) => (
                        <Link
...
function SidebarLink({
  ...
  return (
    ...
    >
      ...
      {hasArrow && <ArrowIcon className="w-3.5 h-3.5 text-[#B0A8A0] opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />}
    </Link>
  )
}
